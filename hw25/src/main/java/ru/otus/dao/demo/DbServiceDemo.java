...
///
        ...
        dbServiceClient.saveClient(new Client("name","login","1"));

        var clientLogin = dbServiceClient.getClientByLogin("login");

        log.info("clientByLogin:{}", clientLogin.toString());
        log.info("All clients");
        dbServiceClient.findAll().forEach(client -> log.info("client:{}", client.toString()));
    }
}
